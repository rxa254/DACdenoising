...
void invert_bqf_double(double* input, double* output, int s, double **sos, double g, int n)
{
    ...
    double in;
    //printf("length: %d\tgain:%g\torder:%d\tin: %g\tout:%g\n",s,g,n,input[0],output[0]);
    for(j=0; j<s; j++) {
        //out = g*input[j];
        in=output[j];
        for(i=0; i<n; i++) {
...
void invert_bqf_longdouble(long double* input, long double* output, int s, double **sos, double g, int n)
{
    int i,j;
    ...
    long double **u = malloc(n*sizeof(long double*));
    for(i=0; i<n; i++) u[i] = malloc (2 * sizeof(long double));
    for(i=0; i<n; i++) for(j=0; j<2; j++) u[i][j] = 0;
