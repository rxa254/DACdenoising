...
long double iir_filter_longdouble(long double input, long double *coef,int n,long double *history)
{

  int i;
  long double *coef_ptr;
  long double *hist1_ptr,*hist2_ptr;
  long double output=0,new_hist,history1,history2;

  coef_ptr = coef;                /* coefficient pointer */
  
  hist1_ptr = history;            /* first history */
  hist2_ptr = hist1_ptr + 1;      /* next history */
//     printf("inside filter: out_mid: %Lg\n",output); 
//     printf("inside filter: in_mid: %Lg\n",input); 
//     printf("inside filter: coef_mid: %Lg\n",*coef_ptr++); 
  output = input * (*coef_ptr++); /* overall input scale factor */
//    printf("inside filter: out_mid2: %Lg\n",output); 
//    printf("inside filter: out_mid: %Lg\n",output); 
  for(i = 0 ; i < n ; i++) {
